...
async function checkForUpdates(currentVersion) {
  ...
  try {
    const response = await fetch('https://registry.npmjs.org/@innerjoint%2Foh-my-claudecode/latest', {
...
async function main() {
  try {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if ((pkg?.name === '@innerjoint/oh-my-claudecode' || pkg?.name === 'oh-my-claudecode') && pkg?.version) {
...
- "Upgrade now" (Recommended): Run \`npm install -g @innerjoint/oh-my-claudecode@latest\` via Bash, then run \`omc install --force --skip-claude-check --refresh-hooks\` to reconcile hooks and CLAUDE.md
